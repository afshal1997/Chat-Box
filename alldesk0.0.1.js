...
function sendMessageAndAction(parent) {
    ...
    input_group_append_message_addon.innerHTML = '<img style="height:20px;width:100%" src="/Assets/Images/fas_fa_paper_plane.svg" alt="sen-message-icon">'
...
function chat_attachments(parent) {
    ...
    const fa_fa_paperclip_attachment = document.createElement('span')
    ...
    fa_fa_paperclip_attachment.innerHTML='<img style="height:20px;height:20px; margin-bottom:5px"  src="/Assets/Images/fa_fa_paperclip_attachment.svg" alt="sen-message-icon">'
...
function enableHeadCssScripts() {
    const headerLists = [
        { href: "Assets/Css/all_desk_chat_app.css", rel: 'stylesheet' }
    ]
    headerLists.forEach((scriptsDetails) => {
        const linkTag = document.createElement('link')
        linkTag.setAttribute('href', scriptsDetails.href)
        linkTag.setAttribute('rel', scriptsDetails.rel)
        document.head.appendChild(linkTag)
    })
}
enableHeadCssScripts()
chatInit()